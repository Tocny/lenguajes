...
(test (prueba '{fun {{x : number} {y : boolean}} : number {if y x 0}}) (funT (list (numberT) (booleanT) (numberT))))

(test/exn (prueba '{fun {{x : number} {y : number}} : number {if y x 0}}) "if: Type error\nConditional's type must be a boolean\nGiven: (numberT)")

(test (prueba '{{fun {{x : number} {y : boolean}} : number {if y x 0}} {2 #t}}) (numberT))

(test/exn (prueba '{{fun {{x : number} {y : boolean}} : number {if y x 0}} {2 3}})
